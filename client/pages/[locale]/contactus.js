        ...
        <main className="mx-auto max-w-7xl my-8">
            <div className="flex grid grid-cols-1 sm:grid-cols-2 md:grid-cols-7 mx-4 sm:mx-5 lg:mx-6 sm:gap-4 md:gap-6 lg:gap-16">
                <div className="sm:order-last md:col-span-4">
                    <h3 style={{zIndex:'-999'}}
                        className={router.query?.locale == 'fa' ? "text-base font-extrabold block relative before:content{''} before:absolute before:right-0  before:top-9 before:h-1 before:w-32 before:bg-[#5269EA]" : "text-base font-extrabold block relative before:content{''} before:absolute before:left-0  before:top-9 before:h-1 before:w-32 before:bg-[#5269EA]"}>
                         {t(router.query?.locale, "counseling_request")}</h3>
                    <form className="mt-8">
                        <div className="flex grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div className="mb-4">
                                <label className="text-sm mb-2 block">
                                {t(router.query?.locale, "i_name_family")}
                                </label>
                                <input type="text" placeholder={t(router.query?.locale, "p_name_family")} className="w-full h-12 px-3 py-2 text-sm placeholder:text-xs placeholder:text-zinc-500 rounded focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500" />
                            </div>
                            <div>
                                <label className="text-sm mb-2 block">{t(router.query?.locale, "i_email")}</label>
                                <input type="email" placeholder={t(router.query?.locale, "p_email")} className="w-full h-12 px-3 py-2 text-sm placeholder:text-xs placeholder:text-zinc-500 rounded focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500" />
                            </div>
                        </div>
                        <div className="mt-4">
                            <label className="text-sm mb-2 block">{t(router.query?.locale, "i_description")}</label>
                            <textarea placeholder={t(router.query?.locale, "p_description")} className="resize-none w-full h-32 px-3 py-2 text-sm placeholder:text-xs placeholder:text-zinc-500 rounded focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 "></textarea>
                    </div>
                    <button type="submit" className="text-sm text-white transition-all bg-[#4F66E8] px-5 py-3.5 mt-6 rounded hover:bg-[#728AF9]">{t(router.query?.locale, "b_send")} </button>
                    </form>
                </div>
                <div className="mt-9 sm:mt-0 sm:order-first md:col-span-3">
                    <h3
                    className={router.query?.locale == 'fa' ? "text-base font-extrabold block relative before:content{''} before:absolute before:right-0  before:top-9 before:h-1 before:w-32 before:bg-[#5269EA]" : "text-base font-extrabold block relative before:content{''} before:absolute before:left-0  before:top-9 before:h-1 before:w-32 before:bg-[#5269EA]"}>
                   {t(router.query?.locale, "connection_ways")}
                </h3>
                <p className="text-sm mt-8 leading-8">
                {t(router.query?.locale, "connection_text")}
                </p>
                <ul className="mt-5">
